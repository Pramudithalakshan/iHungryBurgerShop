...
class iHungryBurger {
	...
	public static void searchCustomerDetails() {
		...
		if (validatePhoneNo(p)) {
			if (cusId.length == 0) {
				...
				for (int i = 0; i < orIdOfCus.length; i++) {
					if (orIdOfCus[i] == phone) { 
						System.out.printf("%-12s %-15d %-12.2f%n",
								orderId[i], orderQty[i], orderQty[i] * UNIT_PRICE);
					}
				}
				System.out.println("---------------------------------------------");
				L2: while (true) {
	...
	public static void updateStatus(int customerId, String orderid, int orIndex) {
		...
		UO: while (true) {
			...
			if (value == 0) {
				...
				quantity[customerIdIndex] = quantity[customerIdIndex] - orderQty[orIndex]; // remove quantity if order
																							// cancelled
			} else if (value == 1) {
				status[orIndex] = 0;
			} else if (value == 2) {
	...
	public static void updateQuantity(int customerId, String orderId, int orIndex) {
		...
		int customerIdIndex = findIndexCusId(customerId);
		quantity[customerIdIndex] = quantity[customerIdIndex] - orderQty[orIndex] + value;
		System.out.println();
		L2: while (true) {
			int serviceDeci = anotherService2();
			switch (serviceDeci) {
